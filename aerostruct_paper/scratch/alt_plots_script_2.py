...
if rank == 0:
    ...
    plt.gca().set_ylim(top=10 ** math.ceil(math.log10(max([ehmm[0], ekmm[0], eamm1[0],ehmm1[0],ehmm2[0]]))))
    plt.gca().set_ylim(bottom=10 ** math.floor(math.log10(min([ehmm[-1], ekmm[-1], eamm1[-1],ehmm1[-1],ehmm2[-1]]))))
    plt.xticks(ticks=np.arange(min(samplehist), max(samplehist), 40), labels=np.arange(min(samplehist), max(samplehist), 40) )
    plt.grid()
    ax.xaxis.set_minor_formatter(mticker.ScalarFormatter())
    ax.xaxis.set_major_formatter(mticker.ScalarFormatter())
    ...
    plt.gca().set_ylim(top=10 ** math.ceil(math.log10(max([ehsm[0], eksm[0], easm1[0],ehsm1[0],ehsm2[0]]))))
    plt.gca().set_ylim(bottom=10 ** math.floor(math.log10(min([ehsm[-1], eksm[-1], easm1[-1],ehsm1[-1],ehsm2[-1]]))))
    plt.xticks(ticks=np.arange(min(samplehist), max(samplehist), 40), labels=np.arange(min(samplehist), max(samplehist), 40) )
    plt.grid()
    ax.xaxis.set_minor_formatter(mticker.ScalarFormatter())
    ax.xaxis.set_major_formatter(mticker.ScalarFormatter())
