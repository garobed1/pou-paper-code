...
def genSlice(s, sl):
    ...
    weights = np.ones(total)#total
    for i in range(total):
        dist[i][0] = saconstsm[0]
        dist[i][1] = saconstsm[1]
        dist[i][2] = saconstsm[2]
        dist[i][3] = saconstsm[3]
        dist[i][sl] = (x[i])*(bounds[sl][1]-bounds[sl][0]) + bounds[sl][0]
