...
class PlateComponentLHS(om.ExplicitComponent):
    ...
    def initialize(self):
        ...
        self.DVCon.setDVGeo(self.solvers.DVGeo.getFlattenedChildren()[1])
        self.DVCon2.setDVGeo(self.solvers.DVGeo)

        self.DVCon.setSurface(self.solvers.getTriangulatedMeshSurface(groupName='allSurfaces'))
        # set extra group for surface area condition
        self.DVCon2.setSurface(self.solvers.getTriangulatedMeshSurface(), name='wall')

        # DV should be same into page (not doing anything right now)
        #import pdb; pdb.set_trace()
        lIndex = self.solvers.DVGeo.getFlattenedChildren()[1].getLocalIndex(0)
        indSetA = []
        indSetB = []
        nXc = optOptions['NX']
    ...
    def setup(self):
        ...
        a_init = self.solvers.DVGeo.getValues()
        a_init['pnts'][:] = self.ooptions['DVInit']
